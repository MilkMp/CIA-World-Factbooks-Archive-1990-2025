...
abstract: |
  ...
  \vspace{0.3cm}
  \noindent\textbf{Key Metrics:} 281 Entities | 36 Editions | 9,536 Country-Year Records | 1,071,603 Data Points
...
header-includes:
  ...
  - \fancyhead[R]{\small Project Report --- March 2026}
  - \fancyfoot[C]{\thepage}
...
| Feb 28 | v3.2 release (pipe delimiters, SourceFragment provenance, 18 new parsers); CSI Studies in Intelligence integration; COCOM boundaries |
...
- **Public archive** (`CIA-World-Factbooks-Archive-1990-2025`): ETL pipeline, raw data, SQLite database, documentation, GitHub Pages landing site. 63 commits.
- **Private webapp** (`cia-factbook-webapp`): FastAPI application, templates, static assets, deployment configuration. 142 commits.
...
\node[block, right=1.0cm of parse] (canon) {Canonicalization\\1,090 $\rightarrow$ 416};
\node[block, left=1.0cm of parse] (detect) {Format\\Detection};

% Database
\node[dbblock, below=1.0cm of parse] (db) {SQLite Database\\factbook.db (324 MB)};
...
The canonicalization layer (described in Part III) maps all 1,090 field name variants to 416 canonical names.
...
\node[proc, left=0.6cm of subfield] (canon) {Canonicalization\\1,090 $\rightarrow$ 416};
\node[proc, left=0.6cm of canon] (fts) {FTS5 Index\\Construction};

% Database
\node[db, below=0.8cm of canon] (database) {factbook.db\\324 MB};
...
Across 36 years of publication, the CIA used 1,090 distinct field name variants for what are effectively 416 unique concepts.
...
The archive uses a normalized relational schema in SQLite, organized around five core tables.
...
\centering
\footnotesize
\caption{Database schema --- five core tables with row counts and relationships.}
\label{tab:erd}
...
\textbf{FieldNameMappings} \newline (1,090) & MappingID (PK), OriginalName, CanonicalName, MappingType, IsNoise, FirstYear, LastYear, UseCount & Logical join on FieldName $=$ OriginalName \\[6pt]
...
Rather than creating a separate sub-field table (which would multiply the row count by 3--5x), sub-field values are stored as pipe-delimited strings within the `Content` column.
The webapp's query layer parses these at read time.

### SourceFragment Provenance

Version 3.2 added a `SourceFragment` column to `CountryFields`, preserving the original raw text from the source file before parsing.
This enables auditing of parse accuracy and debugging of extraction errors.
...
- **Size:** The complete database is 324 MB, small enough to bundle in the Docker image.
...
\node[sublabel, anchor=west] at (6.0, -1.0) {324 MB + FTS5};
...
The landing page presents a global search bar, summary statistics (281 entities, 36 editions, 9,536 records, 1,071,603 data points), and navigation cards linking to the major sections of the application.
...
The ETL pipeline maps 1,090 distinct raw field name variants to 414 canonical names, achieving a **2.63:1 compression ratio**.
The mapping rules break down as follows:

| Rule | Mappings | Percentage | Description |
|------|---------|------------|-------------|
| Country-Specific | 354 | 32.5% | Fields unique to individual countries (e.g., governance bodies) |
| Noise | 281 | 25.8% | Parser artifacts, fragments, sub-field text captured as field names |
| Identity | 184 | 16.9% | Modern field names unchanged from source |
| Rename | 159 | 14.6% | CIA vocabulary changes across decades |
| Dash Format | 64 | 5.9% | Inconsistent dash/hyphen formatting normalized |
| Consolidation | 48 | 4.4% | Multiple sub-fields merged under parent field |

The high proportion of country-specific fields (32.5%) reflects the 1990s-era Factbook practice of including locally relevant fields that were later standardized.
The noise detection rate (25.8%) validates the importance of automated quality filtering --- without it, over a quarter of the field vocabulary would pollute query results.
...
The project was completed in a 14-day intensive development sprint, producing:

| Metric | Value |
|--------|-------|
| Total commits | 205 |
| Average commits per day | 14.6 |
| Archive repository commits | 63 (Feb 16--28) |
| Webapp repository commits | 142 (Feb 22--Mar 2) |
...
| Source fragment provenance (FieldValues table) | 1,610,973 / 1,610,973 | 100.0\% |
...
The canonicalization pipeline (`etl/canonicalize.py`) maps 1,090 raw field name variants to 414 canonical names.
The mapping was validated through the following process:

1. **Noise filtering:** 281 mappings (25.8\%) were identified as parser artifacts and excluded from the canonical vocabulary, preventing query pollution
2. **Rename verification:** 159 mappings (14.6\%) represent CIA vocabulary changes across decades (e.g., "National product" $\rightarrow$ "GDP", "Communists" $\rightarrow$ retired). Each was manually reviewed against source documents
3. **Consistency check:** 184 mappings (16.9\%) are identity mappings where modern field names pass through unchanged, confirming parser accuracy for contemporary formats
4. **Consolidation audit:** 48 mappings (4.4\%) merge sub-field variants under parent fields, each verified against the CIA's own field restructuring timeline

The resulting canonical vocabulary achieves a **2.63:1 compression ratio** while preserving all semantically distinct information.
...
The ETL pipeline extracts 1,610,973 sub-field values from the 1,071,603 parent records, yielding an average of **1.5 sub-fields per record**.
...
The archive's data quality profile --- 100\% content completeness, 100\% provenance tracing, 100\% encoding integrity, and 76.5\% full temporal coverage --- exceeds the standards typical of government open-data repositories.
...
- **Numeric value extraction.** The indicator system extracts numeric values from free-text fields using regex patterns. Unusual formatting (e.g., "approximately 3 million" vs. "3,000,000") may cause extraction failures for specific country-year combinations.
...
- **Structured sub-field expansion.** Parse additional free-text fields (e.g., "Military branches," "Political parties and leaders") into structured, queryable sub-fields.
