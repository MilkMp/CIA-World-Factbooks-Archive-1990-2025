...
Per-field format: each (country, field) pair is a separate dictionary entry,
matching the KOReader convention (one lookup = one fact). For example,
"Afghanistan - Population" is its own entry.

Generates two editions per year (1990-2025):
  - General:    Full field text (one entry per country+field)
  - Structured: Parsed numeric sub-values with units (one entry per country+field)
...
def build_country_codes(name, iso, fips, iso_owner=None):
    """Return list of name/code prefixes for a country.
...
def build_field_headwords(country_codes, field_name):
    """Build headwords for a per-field entry.

    Returns: ["Country Name - Field Name", "ISO - Field Name", ...]
    """
    return [f"{code} - {field_name}" for code in country_codes]
...
# ── Entry builders ──────────────────────────────────────────────────
def build_general_field_html(country_name, category, content):
    """Build HTML for a single general field entry."""
    parts = [
        f"<i>{esc(country_name)}</i><br>",
        f"<small>{esc(category)}</small><br><br>",
        esc(content),
    ]
    return "\n".join(parts)


def build_structured_field_html(country_name, category, field_name, sub_rows):
    """Build HTML for a single structured field entry.

    sub_rows: [(SubField, NumericVal, Units, TextVal), ...]
    """
    parts = [
        f"<i>{esc(country_name)}</i><br>",
        f"<small>{esc(category)}</small><br><br>",
    ]
    for subfield, numval, units, textval in sub_rows:
        val_str = format_numeric(numval, units)
        if val_str is None and textval:
            val_str = esc(textval[:500]) + ("..." if len(textval) > 500 else "")
        if val_str:
            parts.append(f"<b>{esc(subfield)}</b>: {val_str}<br>")
    return "\n".join(parts)
...
def write_stardict(entries, year, edition, output_dir, dictzip=True):
    ...
    glos.setInfo(
        ...
        f"{len(entries)} entries (one per country+field). "
...
def build_general_dict(db, master, year, output_dir, dictzip, iso_owner=None):
    """Build General edition for one year (per-field entries). Returns entry count or 0."""
    t0 = time.time()
    ...
    # Group by (CountryID, FieldName) so each field is its own entry.
    # field_key -> (period_name, mcid, category, content)
    field_entries = OrderedDict()
    for coid, mcid, period_name, cat, field, content, _cid, _fid in rows:
        if not content or not content.strip():
            continue
        key = (coid, field)
        if key in field_entries:
            # Same field appearing twice (rare): append content
            prev = field_entries[key]
            field_entries[key] = (prev[0], prev[1], prev[2], prev[3] + "\n" + content)
        else:
    ...
    print(f"  {year} general:     {len(entries):>5} entries ({elapsed:.1f}s)")
    return len(entries)


def build_structured_dict(db, master, year, output_dir, dictzip, iso_owner=None):
    """Build Structured edition for one year (per-field entries). Returns entry count or 0."""
    t0 = time.time()
    ...
    # Group by (CountryID, FieldName) -> list of sub-values
    field_entries = OrderedDict()
    field_meta = {}  # (coid, field) -> (period_name, mcid, category)
    for coid, mcid, period_name, cat, field, sub, numval, units, textval, _cid, _fid, _vid in rows:
        key = (coid, field)
        field_entries.setdefault(key, []).append((sub, numval, units, textval))
    ...
    elapsed = time.time() - t0
    print(f"  {year} structured:  {len(entries):>5} entries ({elapsed:.1f}s)")
    return len(entries)
